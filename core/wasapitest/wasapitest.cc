...
void PlayerThread(std::atomic_bool& exit) {
  ...
  hr = audio_device->Activate(__uuidof(IAudioClient3), CLSCTX_ALL, NULL,
                              &audio_client);
  assert(SUCCEEDED(hr));
  ...
  hr = audio_client->IsFormatSupported(share_mode, (WAVEFORMATEX*)&wfx,
                                       &matched_wfx);
  assert(SUCCEEDED(hr));

  hr = audio_client->Initialize(share_mode, stream_flags, buffer_duration,
                                hns_periodicity, (WAVEFORMATEX*)&wfx, NULL);
  assert(SUCCEEDED(hr));
